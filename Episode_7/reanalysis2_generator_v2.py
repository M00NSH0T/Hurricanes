...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, years_per_epoch=1,batch_size=32,normalize=True,normalization_mode='min_max',padded=True,centered=True):
        'Initialization'
        ...
        self.lats=np.arange(90,-90.1,-2.5)
        self.lons=np.arange(0,360,2.5)
...
#        self.__reset__()
    ...
    def __load_year__(self,year):
        ...
        with open(full_path, 'rb') as pickle_file:
            ...
            if np.ma.is_masked(fulldataset):
                fulldataset=fulldataset.filled(0)
        if len(self.data)<=1:#==[] deprecated
            self.data=fulldataset #catches centered pkl files, which already were converted from masked arrays.
        else:
            self.data=np.concatenate((self.data,fulldataset),axis=1)
...
#    def __iter__(self):
        ...
        if self.sample_tracker['weights'].sum()>self.batch_size:
            indices=self.sample_tracker['indices'].sample(n=self.batch_size,weights=self.sample_tracker['weights']).values
        else:
            indices=self.sample_tracker['indices'].sample(n=self.batch_size).values
